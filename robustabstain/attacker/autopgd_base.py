...
def L1_projection(x2, y2, eps1):
    ...
    c5 = s1 + c < 0
    # c2 = c5.nonzero().squeeze(1) # deprecated overload
    c2 = torch.nonzero(c5, as_tuple=True)[0].squeeze(1)
    ...
    if c2.nelement != 0:
      ...
      while counter < nitermax:
        ...
        # ind3 = c8.nonzero().squeeze(1) # deprecated overload
        ind3 = torch.nonzero(c8, as_tuple=True)[0].squeeze(1)
        # ind32 = (~c8).nonzero().squeeze(1) # deprecated overload
        ind32 = torch.nonzero(~c8, as_tuple=True)[0].squeeze(1)
        #print(ind3.shape)
        if ind3.nelement != 0:
...
class APGDAttack():
    ...
    def conf_loss(self, x, y):
        probs = F.softmax(x, dim=1)

        # find top adv labels that are NOT equal to the base prediction
        top2_adv_labels = torch.argsort(x, dim=1, descending=True)[:, :2] # top2 predicted labels
        adv_targets = torch.where(top2_adv_labels[:, 0] == y, top2_adv_labels[:, 1], top2_adv_labels[:, 0])

        return -1 * F.cross_entropy(x, adv_targets, reduction='none')
    ...
    def attack_single_run(self, x, y, x_init=None):
        ...
        if not self.is_tf_model:
            if self.loss == 'ce':
                ...
                criterion_indiv = self.conf_loss
            else:
        ...
        for i in range(self.n_iter):
            ...
            acc_steps[i + 1] = acc + 0
            # ind_pred = (pred == 0).nonzero().squeeze() # deprecated overload
            ind_pred = torch.nonzero(pred == 0, as_tuple=True)[0].squeeze()
            ...
            with torch.no_grad():
              ...
              loss_steps[i] = y1 + 0
              # ind = (y1 > loss_best).nonzero().squeeze() # deprecated overload
              ind = torch.nonzero(y1 > loss_best, as_tuple=True)[0].squeeze()
              ...
              if counter3 == k:
                  if self.norm in ['Linf', 'L2']:
                      ...
                      if fl_oscillation.sum() > 0:
                          # ind_fl_osc = (fl_oscillation > 0).nonzero().squeeze() # deprecated overload
                          ind_fl_osc = torch.nonzero(fl_oscillation > 0, as_tuple=True)[0].squeeze()
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        assert self.loss in ['ce', 'dlr', 'conf'] #'ce-targeted-cfts'
        ...
        if not best_loss:
            ...
            for counter in range(self.n_restarts):
                # ind_to_fool = acc.nonzero().squeeze() # deprecated overload
                ind_to_fool = torch.nonzero(acc, as_tuple=True)[0].squeeze()
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    # ind_curr = (acc_curr == 0).nonzero().squeeze() # deprecated overload
                    ind_curr = torch.nonzero(acc_curr == 0, as_tuple=True)[0].squeeze()
            ...
            for counter in range(self.n_restarts):
                ...
                # ind_curr = (loss_curr > loss_best).nonzero().squeeze() # deprecated overload
                ind_curr = torch.nonzero(loss_curr > loss_best, as_tuple=True)[0].squeeze()
