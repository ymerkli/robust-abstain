...
from robustabstain.utils.datasets import CIFAR10

class SemiSupervisedDataset(Dataset):
    """
    Datasets with unlabeled (or pseudo-labeled) data (source: https://github.com/yaircarmon/semisup-adv)
    """
    def __init__(
            self,
        ...
        if dataset is not None:
            ...
            if base_dataset == 'cifar10':
                self.dataset = CIFAR10(train=train, root=root, **kwargs)
            elif base_dataset == 'svhn':
                if train:
                    self.dataset = torchvision.datasets.SVHN(split='train', root=root, **kwargs)
                else:
                    self.dataset = torchvision.datasets.SVHN(split='test', root=root, **kwargs)
                # because torchvision is annoying
                self.dataset.targets = self.dataset.labels
                self.targets = list(self.targets)

                if train and add_svhn_extra:
                    svhn_extra = torchvision.datasets.SVHN(split='extra', **kwargs)
        ...
        if self.train:
            ...
            if aux_data_filename is not None:
                aux_path = os.path.join(root, aux_data_filename)
