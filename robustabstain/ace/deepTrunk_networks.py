...
from robustabstain.ace.layers import Linear, Sequential, Entropy, add_bounds
from robustabstain.ace.networks import SeqNet
from robustabstain.ace.relaxed_networks import CombinedNetwork
from robustabstain.ace.utils import get_net, init_slopes, load_net_state
from robustabstain.ace.zonotope import HybridZonotope, clamp_image
...
class MyDeepTrunkNet(torch.nn.Module):
    def __init__(self, device, args, dataset, trunk_net, input_size, input_channel, n_class, n_branches, gate_type,
                 branch_net_names, gate_net_names, evalFn, lossFn, model_path=None):
        ...
        if model_path is not None:
            ...
            print("Whole model loaded from %s" % model_path)
        ...
        if (model_path is not None or args.load_gate_model is not None) and args.gate_feature_extraction is not None:
    ...
    @staticmethod
    def get_deepTrunk_net(args, device, lossFn, evalFn, input_size, input_channel, n_class, trunk_net=None, model_path=None):
        ...
        specNet = MyDeepTrunkNet(device, args, args.dataset, trunk_net, input_size, input_channel, n_class,
                                 args.n_branches, args.gate_type, args.branch_nets, args.gate_nets, evalFn, lossFn, model_path)
